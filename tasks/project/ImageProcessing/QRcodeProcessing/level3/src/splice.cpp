...
Mat spliceQRcode(std::vector<Mat> quarters)
{
		...
		if(found != -1)
                {
                        ...
                        if(feature.x < 90 && feature.y < 90)
                                position[k] = 1;
                        else if(feature.x > 90)
                                position[k] = 2;
                        else
                                position[k] = 3;
                }
	...
	//拼接
        Mat dst(330, 330, CV_8UC1, Scalar(0));
        img1.copyTo(dst(Rect(0, 0, 180, 180)));
        img2.copyTo(dst(Rect(150, 0, 180, 180)));
	img3.copyTo(dst(Rect(0, 150, 180, 180)));
        img4.copyTo(dst(Rect(150, 150, 180, 180)));
