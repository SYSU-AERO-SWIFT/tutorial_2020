...
Mat preprocess(Mat& src)
{
	...
	Mat r_channelBinary;
	threshold(r_channelGauss, r_channelBinary, 180, 255, THRESH_BINARY);
...
{
	Mat copy = src.clone();
	fastNlMeansDenoisingColored(src, copy);
	Mat hsv;
	cvtColor(copy, hsv, COLOR_BGR2HSV);
	vector<Mat> channels_hsv;
	split(hsv, channels_hsv);
	equalizeHist(channels_hsv[1], channels_hsv[1]);
	merge(channels_hsv, hsv);
	...
	/*
	Mat grayscale_src;
	cvtColor(src, grayscale_src, COLOR_BGR2GRAY);
	if (grayscale_src.channels() != 1)
	{
		return;
	}
	threshold(grayscale_src, grayscale_src, 200, 255, THRESH_BINARY);
	imshow("temp", grayscale_src);
	waitKey(0);
	Mat stats;
	Mat centroid;
	connectedComponentsWithStats(grayscale_src, grayscale_src, stats, centroid);
	double sum_area = 0;
	for (int i = 0; i < stats.rows; i++)
	{
		sum_area += stats.at<int>(i, 4);
	}
	double average_area = sum_area / stats.rows;
	for (int i = 0; i < stats.rows; i++)
	{
		if (stats.at<int>(i, 4) > average_area)
		{
			Mat mask = (grayscale_src == i);
			illuminationChange(copy, mask, dst);
		}
	}
	imshow("temp", dst);
	waitKey(0);
